...
const LastReportsPage: React.FC = () => {
  ...
  const loadPageData = useCallback(() => {
    dispatch({ type: GET_LAST_REPORTS });
  }, [dispatch]);

  useEffect(() => {
    loadPageData();
    const interval = setInterval(loadPageData, REFRESH_DATA_INTERVAL);
    return () => {
  ...
  const onSearch = useCallback(
    (value: string) => {
      ...
      const newSorted = reports.filter((report) => {
        return (
          String(report.test_name).toLowerCase().includes(value.toLowerCase()) ||
          String(report.status).toLowerCase().includes(value.toLowerCase())
        );
      });
  ...
  const noDataText = processing ? undefined : 'There is no data';
